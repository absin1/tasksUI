    ...
    fileprivate func adjustScrollView() {
        scrollView.contentSize =
            CGSize(width: scrollView.frame.width * CGFloat(numPages),
                   height: scrollView.frame.height - (view.safeAreaInsets.bottom-view.safeAreaInsets.top))
    }
    ...
    fileprivate func loadPage(_ page: Int) {
        ...
        if pages[page] == nil {
            if let image = UIImage(named: "\(page + 1)") {
                ...
                frame.origin.y = -(view.safeAreaInsets.bottom-view.safeAreaInsets.top)
                frame.size.height += (view.safeAreaInsets.bottom-view.safeAreaInsets.top)
